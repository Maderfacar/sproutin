...
> Last updated: 2026-08-18（**打磨第二階段開工：Band 元件 + 聯絡簿 IMPLEMENTED**——
> 版面斷句 + 多重身分標示。三題由 Human Owner 定案（區塊元件／只給多重身分看／六頁）。
> **剩下 5 頁（娃娃車、出缺勤、請假、公告、學生整合視圖）未做，已交接。**
>
...
**打磨第二階段 —— 版面斷句 + 多重身分標示（Band 元件 + 聯絡簿）
— IMPLEMENTED / VERIFICATION_PENDING（2026-08-18）。**
四項全綠：lint / typecheck / **測試 522（api 362 + shared 12 + web 148）** / build。
無 migration、無新後端端點。
...
```text
已完成  components/Band（kind=action/review/manage + title + description + audience）
        roleFlags 新增 hasDualIdentity（校方 ∧ 家長）—— 仍是唯一的顯示判斷來源
        聯絡簿改成範本頁：老師的記錄區在上（粗線 + 以老師身分）、翻閱區在下（細線收斂）
未完成  娃娃車 / 出缺勤 / 請假 / 公告 / 學生整合視圖 —— 5 頁，已交接給新視窗
