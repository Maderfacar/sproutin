...
- Step 7 — Dashboard·Branding·Feature Flag（前端可操作頁面,切子步驟先家長→老師→園長）→ **IN_PROGRESS**。
  ...
  - 7d 園長/ADMIN（稽核查詢頁 + 全校公告 + 全校待審總覽 + GET /leaves 全校模式）→ ✅ **ACCEPTED**（2026-08-16）。
- **Phase 7 = COMPLETE**（Step 1–7 全 ACCEPTED;LINE 推播線上實測收到、帶學生姓名）。
- 下一步：**Phase 8 — Integration / Hardening**（新 session,先計畫→確認）。候選:ESLint、append-only DB 鎖死(§D)、全域 exception filter 統一信封、web 元件測試、多校隔離/secret/效能、P5 demo 資料收尾（園長 ADMIN+監護 hack）。
```
