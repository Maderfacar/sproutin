...
- Step 7 — Dashboard·Branding·Feature Flag（前端可操作頁面,切子步驟先家長→老師→園長）→ **IN_PROGRESS**。
  - 7a 前端地基（Tailwind + TanStack Query + runtime 品牌）+ 家長「請假」端到端 → **IMPLEMENTED**（2026-08-16;本機 typecheck/test[133]/build 綠;待 push→CI + Vercel + Human 手機實測）。UI/資料抓取新 library 經 §D 核准;多重身份採聯集視圖（架構本就支援）。
- 下一步：7a 手機實測 acceptance → 7b（家長其餘卡片）。平行未了:append-only 鎖死（下一版 §D）、Step 5 LINE 推播線上實測前置。（Phase 6 全數 ACCEPTED。）
```
