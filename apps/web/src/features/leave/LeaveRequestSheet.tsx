...
//
// 同一份面板兩種用法：
//   家長 —— 只有一個小孩就不畫選擇器（不必按一次確認自己是誰）。
//   校方 —— **代家長請假**（家長打電話來請假是實際會發生的事，docs/05 矩陣
//           `canApplyLeave` 本來就包含 TEACHER/ADMIN）。這時要選是哪個孩子。
...
export function LeaveRequestSheet({
  ...
  const chosen = studentId || defaultStudentId || students[0]?.id || '';
